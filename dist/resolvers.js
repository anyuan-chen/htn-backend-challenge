...
const skillFrequencyResolver = async (parent, args, contextValue) => {
    ...
    if (args.filter) {
        ...
        tmp = tmp.filter((skill) => {
            return (skill._count._all >= args.filter.min &&
                skill._count._all <= args.filter.max);
        });
    }
    return tmp;
};
const userCreateInputResolver = async (parent, args, contextValue) => {
...
const updateUserResolver = async (parent, args, contextValue) => {
    if (args.data.skills) {
...
export { userResolver, skillFrequencyResolver, skillsResolver, userCreateInputResolver, allUserResolver, updateUserResolver, };
